...
http://github.com/Eyeline-Research/Go-with-the-Flow

Reference:
[1] Burgert, Ryan, et al.
    Go-with-the-flow: Motion-controllable video diffusion models using real-time warped noise. CVPR.
'''
...
import json
from VLIPP.utils.helpers import io_from_json, apply_alternating_noise, set_seed
...
def load_sample_cartridge(
    ...
    metadata = rp.gather_vars('sample_path downtemp_noise sample_gif_path sample_video sample_noise noise_downtemp_interp')
...
def run_pipe(
    ...
    print("IMAGE",image)
    print("Prompt", cartridge.prompt)
    
    
    cartridge.noise = apply_alternating_noise(cartridge.noise)
    ...
    export_to_video(video, output_mp4_path, fps=16)
...
def main(
    ...
    data_json_path = os.path.join(data_root, 'json', f'{exp_name}.json')
    data_json = io_from_json(data_json_path)
    
    output_mp4_root = os.path.join(data_root, 'results', exp_name)
    os.makedirs(output_mp4_root, exist_ok=True)
    sample_path = os.path.join(data_root, 'noise_flow', exp_name)
    output_mp4_path = os.path.join(output_mp4_root, 'output.mp4')
    ...
    cartridge_kwargs[0].prompt = data_json['recaption']
    cartridge_kwargs[0].sample_path = sample_path
